...
ORAM::ORAM(int maxSize, bytes<AES_BLOCK_SIZE> oram_key)
    ...
    auto blockCount = (size_t) (Z * (pow(2, depth + 1) - 1));
    maxHeightOfAVLTree = (int) floor(log2(blockCount)) + 1;
    auto storeBlockSize = (size_t) (AES_BLOCK_SIZE + get_cipher_length((int) (Z * (blockSize))));
    size_t storeBlockCount = blockCount;
    clen_size = get_cipher_length((int) (blockSize) * Z);
...
int ORAM::GetNodeOnPath(int leaf, int curDepth) const {
    leaf += bucketCount / 2;
...
Bucket ORAM::ReadBucket(int index) {
    ...
    block buffer = decrypt(key, ciphertext, clen_size);
...
void ORAM::WriteBucket(int index, Bucket bucket) {
    totalWrite++;
    block b = SerialiseBucket(std::move(bucket));
    block ciphertext = encrypt(key, b, clen_size, plaintext_size);
...
void ORAM::FetchPath(int leaf) {
    ...
    for (int d = 0; d <= depth; d++) {
        ...
        for (int z = 0; z < Z; z++) {
            ...
            if (curBlock.id != 0 && (*(int*) (&curBlock.id.id[ID_SIZE - 4])) > dummyValue) { // It isn't a dummy block
...
Node* ORAM::ReadData(const Bid& bid) {
...
void ORAM::WriteData(const Bid& bid, Node* node) {
    cache[bid] = node;
}

// Fetches a block, allowing you to read and write in a block
void ORAM::Access(const Bid& bid, Node*& node, int lastLeaf, int newLeaf) {
    FetchPath(lastLeaf);
    node = ReadData(bid);
    if (node != nullptr) {
        node->pos = newLeaf;
...
void ORAM::Access(const Bid& bid, Node*& node) {
    if (!batchWrite) {
        FetchPath(node->pos);
    }
    WriteData(bid, node);
...
Node* ORAM::ReadNodeFromCache(const Bid& bid) {
    if (bid == 0) {
...
Node* ORAM::ReadNode(const Bid& bid, int lastLeaf, int newLeaf) {
    if (bid == 0) {
        return nullptr;
    }
...
int ORAM::WriteNode(const Bid& bid, Node* node) {
    if (bid == 0) {
        throw runtime_error("Node id is not set");
    }
    if (cache.count(bid) == 0) {
...
void ORAM::finalize(bool find, Bid& rootKey, int& rootPos) {
